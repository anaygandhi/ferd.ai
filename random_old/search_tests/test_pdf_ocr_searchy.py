...
from file_content_extractions.text_extraction import search_in_pdf, pdf_ocr_extraction
...
def create_test_pdf(filename="test.pdf"):
    ...
    pdf.cell(200, 10, txt="Testing OCR with different fonts.", ln=True, align='C')
    pdf.output(filename)

# Testing the PDF search functionality
def test_pdf_ocr_functionality():
    test_pdf = "test.pdf"
    create_test_pdf(test_pdf)
    expected_text = "Hello, this is a test PDF.\nTesting OCR with different fonts."
